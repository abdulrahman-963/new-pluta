...
@Slf4j
public abstract class TenantBranchContextService<E,D> {


    public abstract GenericRepository<E> getGenericRepository();
    public abstract GenericMapper<E,D> getGenericMapper();

    public D findByIdAndTenantIdAndBranchId(Long id) {
        log.debug("Finding {} by id: {}",this.getClass().getName(), id);
        E entity = getGenericRepository().findByTenantIdAndBranchIdAndId(TenantContext.getTenantId(),TenantContext.getBranchId(),id)
